...
export const defaultForm = { production: '', scene: '01', shot: 'A', take: 1, director: '', camera: '', fps: 25, notes: '' };
...
export function recordsToCSV(records) {
  const headers = ['拍攝時間（ISO 8601 / UTC）', '片名', '場次', '鏡號', 'Take', '導演', '攝影', 'FPS', '打板時間碼（當地時間）', '評記', '音效已播放', '備註'];
  const ratings = { unrated: '未評記', ok: 'OK', ng: 'NG' };
  const rows = records.map(record => [new Date(record.timestamp).toISOString(), record.production, record.scene, record.shot, record.take, record.director, record.camera, record.fps, record.timecode, ratings[record.rating], record.soundPlayed ? '是' : '否', record.notes]);
